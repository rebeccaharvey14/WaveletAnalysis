...
leg2 = ax5.legend(bbox_to_anchor=(1.06,0.5), loc='center right', handlelength=0, handletextpad=0, frameon=False, prop={'size': 12}, fontsize=15)
...
cbar = fig.colorbar(cs, ticks=np.logspace(3,9,num=7),format=ticker.LogFormatterSciNotation(),cax=cbar_ax1)
...
ax6.axvspan(bs_start, bs_end, color='gray', alpha=0.2, lw=0)
ax6.axvline(bs_start, color='black', linewidth=0.2)
ax6.axvline(bs_end, color='black', linewidth=0.2)
...
#ax6.text(-0.05,-0.39,'HH:MM\n X ($R_E$)\n Y ($R_E$)\n Z ($R_E$)',horizontalalignment='right',transform=ax12.transAxes)
